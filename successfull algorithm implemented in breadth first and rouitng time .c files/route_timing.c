		...
		if (itry == 1) {
			...
			for (i = 0; i < num_rr_nodes; i++) {
				if (rr_node[i].type == CHANX || rr_node[i].type == CHANY) {
					available_wirelength += rr_node[i].xhigh
...
boolean timing_driven_route_net(int inet, float pres_fac, float max_criticality,
	...
	for (itarget = 1; itarget <= num_sinks; itarget++) {
		...
		while (inode != target_node) {
			...
			if (old_tcost > 0.90 * HUGE_POSITIVE_FLOAT) /* First time touched. */
...
static void add_route_tree_to_heap(t_rt_node * rt_node, int target_node,
	...
	float tot_cost, backward_path_cost, R_upstream,new_back_pcost;
	...
	if (rt_node->re_expand) {
		...
		backward_path_cost = backward_path_cost+new_back_pcost;
...
new_back_pcost = old_back_pcost+new_back_pcost;
}
