...
int lookup_ipin,num_muxes,used_clb_muxes,used_Clb_pins,used_sides;
int N=num_pins*used_sides*num_muxes*lookup_ipin,k;
...
for(int i=0;i<lookup_ipin;i++)
{
   int I=(i*used_clb_muxes*used_sides*used_Clb_pins);
for(int j=0;j<used_clb_muxes;j++)
{
   int J=(j*used_sides*used_Clb_pins);
for(k=0;k<used_sides;k++)
{
   int K=(k*used_Clb_pins);
for(int L=0;L<used_Clb_pins;L++)
{ 

 track_to_pin_lookup[I][J][K+L].nelem++;
